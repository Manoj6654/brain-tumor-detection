...
def create_3d_surface_model(heatmap, confidence, tumor_type):
    h, w = heatmap.shape
    x = np.linspace(0, w, min(w, 80))
    y = np.linspace(0, h, min(h, 80))
    X, Y = np.meshgrid(x, y)
    Z = cv2.resize(heatmap, (len(x), len(y))) * (confidence / 100) * 3

    fig = go.Figure(data=[go.Surface(
        x=X, y=Y, z=Z,
        colorscale='Plasma',
        contours={"z": {"show": True, "usecolormap": True,
                        "highlightcolor": "cyan", "project": {"z": True}}},
        opacity=0.9,
        colorbar=dict(title="Probability", x=1.02),
    )])
    title = f'<b>3D Tumor Surface</b><br><span style="font-size:13px">{tumor_type} | {confidence:.1f}%</span>'
    ...
    try:
        ...
        rows = conn.execute(
            '''SELECT id, patient_name, patient_id, patient_age, patient_gender, tumor_type,
                      confidence, stage, area_pct, location, scan_date
...
def api_scan_detail(scan_id):
    ...
    result = dict(row)
    # Add stage description and precautions
    if result['tumor_type'] and result['tumor_type'] != 'No Tumor' and result['stage']:
        ...
        precautions   = get_precautions(result['tumor_type'], result['stage'], result['confidence'] or 0, result['area_pct'] or 0)
        medication    = get_medication(result['tumor_type'], result['stage'])
